HELLO___SAILOR int random_function_foo(foobar_t xx){
  int xx = RESULT_OK;
  xx = random_function_fobar(db, "random_strg"qweqejiqwejie, 0);
  return xx;
}

__declspec(dllexport)
HELLO__API int random_function_b(
  foobar_t xx,
  foo **foo_bars,
  const random_random_stuffs *scoo
){
  int xx = RESULT_OK;
  random_function_foobar(scoo);
  xx = random_function_foo(db);
  return xx;
}

/*****************************************************************************/
/* what am i even doing here itpointless */
// HELLO__API const char *random_function_(void){ return HELLO__API const char *random_function_(void){ return HELLO__API const char *random_function_(void){
return_HELLO__API
const char *random_function_(void){ return_HELLO__API const char " trolololololololo(void){ return qoiej qweoijqweo jqw eoiqj weoiqj eoiqjw         "WHO_KNOWS_WHAT_D; } "hello char\
\
